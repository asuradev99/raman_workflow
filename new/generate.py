...
def emit_run_all(cfg, b, active_steps, work_dir):
    ...
    phase_scripts = {}  # jobname -> script content, written out by main()
    ...
    if mode == "sbatch_mix":
        ...
        def phase(steps, sbatch_args, jobname):
            script_name = f".phase_{jobname}.sh"
            phase_scripts[script_name] = (
                "#!/bin/bash\n"
                f"cd {work_dir} && source {COMMON_SH}\n"
                + "\n".join(f"run_until_complete {path(st)}" for st in steps) + "\n"
            )
            step_paths = " ".join(path(st) for st in steps)
            return (f'resubmit_until_done "{jobname}" "{sbatch_args}" '
                    f'"{work_dir}/{script_name}" {step_paths}')
...
def main():
    ...
    run_all_content, phase_scripts = emit_run_all(cfg, b, active_steps, work_dir)
    for script_name, content in phase_scripts.items():
        write(os.path.join(work_dir, script_name), content)
    write(os.path.join(work_dir, "run_all.sh"), run_all_content)
